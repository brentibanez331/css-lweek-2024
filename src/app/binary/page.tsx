...
export default function BinaryGame() {
    const [binaryTarget, setBinaryTarget] = useState(() => Cookies.get("binaryTarget") || "");
    const [answer, setAnswer] = useState('')
    const [binaryTargetAnswer, setBinaryTargetAnswer] = useState("");
    ...
    const handleSubmit = () => {
        console.log(answer)
        console.log(binaryTargetAnswer)
        if (answer === binaryTargetAnswer) {
            router.push("/loading?status=1&game=binary")
        } else{
            router.push("/loading?status=0&game=binary")
        }
    }
